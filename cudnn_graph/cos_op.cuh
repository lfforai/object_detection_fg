...
public:
	static cos_op<T>* getObejct(base_op<T>* op1, T aphla_o, string name_o, char* Tensor_des = "")
	{
		...
		cos_op<T>::global_graph->insert_v(result->name_of_op, result);
		return result;
	}
	...
	virtual void backward_function() {
		...
		constant<T>* temp_const = ((constant<T>*)(*(this->x))[1])->function_tensor(CONS_COS, 1, 1);
	...
	virtual void forward_function() {
		...
		T apla2 = 1.0;
		//1 no use  //0 ,dy/dx==1
		constant<T>* temp_const = ((constant<T>*)(*(this->x))[1])->function_tensor(CONS_COS, 1, 0);
