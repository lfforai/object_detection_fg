...
//-----------------------------------------
template <class value_type>
__global__ void half2float_rn_kernel(int size, const half1 *buffIn, value_type *buffOut)
{
	const int idx = BLOCK_SIZE * blockIdx.x + threadIdx.x;
	if (idx >= size) {
		return;
	...
	val.x = __float2half_rn(float(buffIn[idx]));
#else
	value_type val = (value_type)__half2float(buffIn[idx]);
#endif
	buffOut[idx] = val;
}
...
{
	int grid_size = (size + BLOCK_SIZE - 1) / BLOCK_SIZE;
	half2float_rn_kernel<value_type> << <grid_size, BLOCK_SIZE >> > (size, buffIn, buffOut);
	checkCudaErrors(cudaDeviceSynchronize());
}

template void gpu_half2float_rn<float>(int, const half1*, float*);
template void gpu_half2float_rn<double>(int, const half1*, double*);
