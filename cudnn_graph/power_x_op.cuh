...
#ifndef _POWER_X_OP_CUH
#define _POWER_X_OP_CUH

template<class T>  //x^aphla_o
class power_x_op:public base_op<T>
{
 public:
	static power_x_op<T>* getObejct(base_op<T>* op1, T aphla_o, string name_o, char* Tensor_des = "")
	{
		//assume size must be same
		power_x_op<T>* result = new power_x_op<T>;
		...
		power_x_op<T>::global_graph->insert_v_repeat(result->name_of_op, result);
		return result;
	}
	...
	virtual void backward_function(){
		...
		{  // have sons;
		   //cout << "backward start::" << this->name_of_op << endl;
			...
			(*this->dx)[0] = ((constant<T>*)(*(this->x))[0])->function_tensor(CONS_XPOWA, this->alpha, 1);
	...
	virtual void forward_function(){
		...
		//1 no use  //0 ,dy/dx==1
		this->y = ((*this->x)[0])->function_tensor(CONS_XPOWA, this->alpha, 0);
