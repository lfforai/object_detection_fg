...
#ifndef _RUDUCE_AVG_OP_CUH
#define _RUDUCE_AVG_OP_CUH

template<class T>
class reduce_avg_op :public base_op<T>
{
private:
...
#define C_index(N,H,W,C) N*stride_C[0]+ H * stride_C[1]+ W * stride_C[2]+ C * stride_C[3]
	...
	static reduce_avg_op<T>* getObejct(base_op<T>* op1, int* dim_C_o, string name_o, char* Tensor_des = "")
	{
		//assume size must be same
		reduce_avg_op<T>* result = new reduce_avg_op<T>;
		result->alpha = 1.0;
		...
		reduce_avg_op<T>::global_graph->insert_v_repeat(result->name_of_op, result);
		return result;
	}
	...
	virtual void backward_function() {
		...
		if (this->sons_num > 0)
		{
			...
			T num=(T)((float)(this->dy_sum)->length)/((float)(((*this->dx)[0])->length));

			for (int i = 0; i < ((constant<T>*)((*this->x)[0]))->length; i++)
			{
				((constant<T>*)(*this->dx)[0])->x[i] = num*((constant<T>*)(this->dy_sum))->x[index_p[i]];
			}
		}
		else {
			T num = (T)((float)(this->dy_sum)->length) / ((float)(((*this->dx)[0])->length));
			constant<T>* temp = ((constant<T>*)((*this->x)[0]))->copy2ones();
			(*this->dx)[0] = temp->scala_mul(num);
		}
	...
	virtual void forward_function() {
		...
		constant<T>::op_math_reduce(CONS_REDUCE_TENSOR_AVG, (*(this->x))[0], this->y, &alpha, &beta, result);
		free(result);
